...
void translateTrangleFloat(float &x1,float &y1,float & x2,float &y2,float &x3,float &y3,float dx,float dy)
{
	x1 += dx;
	x2 += dx;
	x3 += dx;
...
void scaleTrangle(float &x1,float &y1,float & x2,float &y2,float &x3,float &y3,float sx,float sy)
{
	...
	translateTrangleFloat(x1,y1,x2,y2,x3,y3,-center_x,-center_y);	//平移
	x1 *= sx;
	x2 *= sx;
	x3 *= sx;
	y1 *= sy;
	y2 *= sy;
	y3 *= sy;
	translateTrangleFloat(x1,y1,x2,y2,x3,y3,center_x,center_y);	//复位
}
